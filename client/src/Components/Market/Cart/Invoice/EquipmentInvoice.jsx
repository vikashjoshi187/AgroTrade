...
const dispatch=useDispatch();
            ...
            if (result.isConfirmed) {
                ...
                let payData={
                        ...
                        console.log("payment data",payData);
                         ...
                         let payResult=   await axios.post(USER_REQUESTED_URL+'/equipmentpayment',payData);
                         await window.open(payResult.data,'_blank')
                         
                         console.log("payment result in equipment invoice",payResult);
    ...
    useEffect(()=>{
            (async()=>{
                ...
                {
                    setInvoice('Agro-Equipment'+101);
                }else if(totalOrder.status===200)
                {
                        setInvoice('Agro-Equipment'+100+totalOrder.data.order.length);
                }

            })();
        console.log("order in invoice component from equipment cart",location.state);
                  ...
                  <table class="table table-success  mb-0 ">
                    <thead className="sticky-top" >
                        <tr>
                            ...
                            <th className="fs-6 p-2 text-center">Rent Day</th>
                    ...
                    <tbody>
                        {
                            cart.map((item,i)=>{
                                return(
                                    ...
                                    <td className="fs-6 text-center p-2">{item.productName}</td>
                                    <td className="fs-6 text-center p-2">{item.productDescription}</td>
                                    <td className="fs-6 text-center p-2">{item.price+item.price*0.25}</td>
                                    <td className="fs-6 text-center p-2">{item.quantity}</td> 
                                    <td className="fs-6 text-center p-2">{item.rentDays}</td> 
